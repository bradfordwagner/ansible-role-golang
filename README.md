...
| `go_src_cleanup_enabled` | `false` | Enable post-install removal of `*_test.go` and `*.pem` files from the install `src/` dir |
| `go_src_cleanup_extra_files` | `[src/net/http/internal/testcert/testcert.go, src/crypto/x509/x509_test_import.go]` | Additional paths relative to `go_install_path` to delete after install; only applied when `go_src_cleanup_enabled` is `true` |
